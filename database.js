...
export async function createTable() {
    return new Promise((resolve, reject) => {
        db.transaction(
            (tx) => {
                tx.executeSql("create table if not exists menuitems (id integer primary key not null, uuid text, title text, price text, category text);");
            },
            reject,
            resolve
        );
...
export function saveMenuItems(menuItems) {
    db.transaction((tx) => {
        ...
        menuItems.forEach(({ uuid, title, price, category }) => {
            tx.executeSql("insert into menuitems (uuid, title, price, category) values (?, ?, ?, ?)", [uuid, title, price, category]);
        });
    });
}
...
export async function filterByQueryAndCategories(query, activeCategories) {
    return new Promise((resolve, reject) => {
        db.transaction((tx) => {
            ...
            const categoriesParam = activeCategories.map((c) => c);

            tx.executeSql(
                "select * from menuitems where title like ? and category in (?)",
                [queryParam, categoriesParam],
                (_, { rows }) => {
